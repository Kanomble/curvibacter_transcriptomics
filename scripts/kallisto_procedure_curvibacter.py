...
def kallisto_quant(directory):
    ...
    samples = []
    for file in files:
        if "_paired_out_" in str(file):
            if file.split('_paired_out_')[-1].split('.')[0].split('_R')[0] not in samples:
                samples.append(file.split('_paired_out_')[-1].split('.')[0].split('_R')[0])

    print("[*] length samples: {} --> total files {}".format(len(samples),len(files)))


    for sample in samples:
        fw_file = directory + '/fw_paired_out_'+ sample + '_R1_001.fastq.gz'
        r_file = directory + '/r_paired_out_' + sample + '_R2_001.fastq.gz'
